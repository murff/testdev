<?php
/*
  $Id: easypopulate.php,v 2.9 18/11/2013 

  Designed for osCommerce, Open Source E-Commerce Solutions
  http://www.oscommerce.com

  Copyright (c) 2013 Craig Poole
...
define ('EP_MODEL_NUMBER_SIZE', 64); // default is 12
...
define ('EP_QPBP', false);  // default is false
define ('EP_QPBP_MAX', 6);  // default is 6, maximum number of columns
...
if (EP_QPBP == true) { 
  //$default_these[] = 'v_products_images_htmlcontent';
  for ($i=1;$i<=EP_QPBP_MAX+1;$i++) {
        $default_these[] = 'v_products_qpbp_price' . $i;
        $default_these[] = 'v_products_qpbp_qty_' . $i;
  }
}
...
//		header('Content-type: application/vnd.ms-excel; charset="utf-8"');
	...
	// get data from products images table
						if (EP_QPBP == true) {
							$i = 1;
							$pi_result = tep_db_query("SELECT * FROM ". TABLE_PRODUCTS_PRICE_BREAK."  WHERE products_id= " . $row['v_products_id'] . " limit " . EP_QPBP_MAX);
							while ($pi_row = tep_db_fetch_array($pi_result)) {
								$row['v_products_qpbp_price_'.$i] = $pi_row['products_price'];
								$row['v_products_qpbp_qty_'.$i] = $pi_row['products_qty'];
								$i++;
							}
						}
      ...
      <table width="<?php if (EP_SHOW_EP_SETTINGS == true) { echo '95'; } else { echo '75'; } ?>%" cellpadding="5" cellspacing="0" style="border-collapse:collapse;">
        ...
        <p style="margin-top: 0px;"><!-- Download file links -  Add your custom fields here -->
          ...
          $cells[$tmp_row_count][$tmp_col_count++] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_specials_price', 'show', (!empty($_GET['epcust_specials_price'])?true:false)) . '</td><td class="smallText"> specials' . '</td></tr></table>');
					...
					if (EP_QPBP == true) {
						$cells[$tmp_row_count][$tmp_col_count++] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_qpbp', 'show', (!empty($_GET['epcust_pi'])?true:false)) . '</td><td class="smallText"> add qpbp' . '</td></tr></table>');
					}
          ...
          <table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">
             ...
             MVS Support: <?php echo (EP_MVS_SUPPORT?'true':'false'); ?><br />
						 Products Images: <?php echo (EP_PRODUCTS_IMAGES?'<font color="green">true':'<font color="grey">false'); ?></font><br />  
                          qpbp: <?php echo (EP_QPBP?'<font color="green">true':'<font color="grey">false'); ?></font><br />
...
function ep_create_filelayout($dltype, $attribute_options_array, $languages, $custom_fields){
    ...
    if ($_GET['epcust_status_filter']!='') {
      $sql_filter .= ' and p.products_status = ' . (int)$_GET['epcust_status_filter'];
		...
		if (EP_QPBP == true) { 
			for ($i=1;$i<=EP_QPBP_MAX+1;$i++) {
				$ep_products_layout_product .= '$filelayout[\'v_products_qpbp_price_'.$i.'\'] = $iii++;
																				$filelayout[\'v_products_qpbp_qty_'.$i.'\'] = $iii++;
																												';
			}
		}
    // /////////////////////////////////////////////////////////////////////
