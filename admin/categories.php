  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'update_category':
		...
		$price_a = tep_db_prepare_input($HTTP_POST_VARS['price_a']);
		$price_b = tep_db_prepare_input($HTTP_POST_VARS['price_b']);
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          ...
          $sql_data_array = array('categories_name' => tep_db_prepare_input($categories_name_array[$language_id]),'price_a' =>$price_a,'price_b' =>$price_b);
...
<?php
    ...
    if (isset($HTTP_GET_VARS['search'])) {
      ...
      $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified,cd.price_a,cd.price_b from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' and cd.categories_name like '%" . tep_db_input($search) . "%' order by c.sort_order, cd.categories_name");
    } else {
...
<?php
    ...
    switch ($action) {
      case 'new_category':
		...
		$contents[] = array('text' => '<br />  Price A <br />' . tep_draw_input_field('price_a', $cInfo->price_a, 'size="20"').'$');
		$contents[] = array('text' => '<br /> Price B <br />' . tep_draw_input_field('price_b', $cInfo->price_b, 'size="20"').'$');
      ...
      case 'edit_category':
		...
		$contents[] = array('text' => '<br />  Staples Price <br />' . tep_draw_input_field('price_a', $cInfo->price_a, 'size="20"').'$');
		$contents[] = array('text' => '<br /> Customer Price <br />' . tep_draw_input_field('price_b', $cInfo->price_b, 'size="20"').'$');
