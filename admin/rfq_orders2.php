...
function get_customer_id($id){
  ...
  if (tep_not_null($action)) {
    ...
    switch ($action) {
      case 'update':
        ...
        $manfacturer = tep_db_prepare_input($HTTP_POST_VARS['manfacturer']);
 ...
 //echo $customers_email_info->customer_id;
  $customers_email = tep_db_query("select * from customers where customers_id = '" . (int)$customers_email_info->customer_id. "'");
 ...
 //exit;
      ...
      case 'rfqconfirm':
		...
		$email_order = "The Item NO " .$rfq_id. "Has been Accepted \n" . 
                 EMAIL_SEPARATOR . "\n\n" .
 		 'View Order:' . "\n" .EMAIL_SEPARATOR . "\n<a href='http://itemnet.ca/cartest/vieworder.php?po=" .$rfq_id. "'>http://itemnet.ca/cartest/vieworder.php?po=" .$rfq_id. "</a>\n\n" ;
	...
	tep_mail($customer_address['customers_firstname'] . ' ' . $customer_address['customers_lastname'], $customer_address['customers_email_address'], 'Quote Order Accepted', $email_order, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
      ...
      /*  tep_db_query("delete from " . TABLE_ADDRESS_BOOK . " where rfq_id = '" . (int)$rfq_id . "'");
		...
		$email_order = "The Post Order " .$rfq_id. "Has been Declined \n" . 
                 EMAIL_SEPARATOR . "\n\n" .
 		 'View Order:' . "\n" .EMAIL_SEPARATOR . "\n<a href='http://itemnet.ca/cartest/vieworder.php?po=" .$rfq_id. "'>http://itemnet.ca/cartest/vieworder.php?po=" .$rfq_id. "</a>\n\n" ;
...
<?php
    ...
    //echo tep_draw_input_field('notes', $cInfo->notes, 'maxlength="32"', false);
	echo tep_draw_textarea_field('notes', 'no', '50', '10', $text = '', $parameters = $cInfo, $reinsert_value = true);
...
<?php
    ...
    while ($customers = tep_db_fetch_array($customers_query)) {
      ...
      if (isset($cInfo) && is_object($cInfo) && ($customers['rfq_id'] == $cInfo->rfq_id)) {
	   ...
	   $customer_address_query = tep_db_query("select c.customers_id, c.customers_firstname, c.customers_lastname, c.customers_email_address from customers c  where c.customers_id = '" . (int)$cInfo->customer_id. "'");
