...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled (or the session has not started)
  ...
  if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'process') && isset($HTTP_POST_VARS['formid']) && ($HTTP_POST_VARS['formid'] == $sessiontoken)) {
    ...
    $email_address = tep_db_prepare_input($HTTP_POST_VARS['user_name']);
    $password = tep_db_prepare_input($HTTP_POST_VARS['password']);

// Check if email exists
    $check_customer_query = tep_db_query("select customers_id, customers_firstname,customers_username, customers_password, customers_email_address, customer_group_id, customers_default_address_id from " . TABLE_CUSTOMERS . " where customers_username = '" . tep_db_input($email_address) . "'");
  ...
  // Check that password is good
        if ($check_customer['customers_password']!= md5($password)){
            $error = true;
        } 
        else{
                if (SESSION_RECREATE == 'True'){
                    //set in db session config 
...
<?php /*?><div class="contentContainer" style="width: 45%; float: left;">
...
<div class="contentContainer" style="width: 45%; float: left; border-left: 0px dashed #ccc; padding-left: 3%; margin-left: 3%;">
  <h2><?php //echo HEADING_RETURNING_CUSTOMER; ?></h2>

  <div class="contentText">
    <p><?php //echo TEXT_RETURNING_CUSTOMER; ?></p>

    <?php echo tep_draw_form('login', tep_href_link(FILENAME_LOGIN, 'action=process', 'SSL'), 'post', 'autocomplete="off"', true); ?>

    <table border="0" cellspacing="0" cellpadding="2" width="100%">
      <tr>
        <td class="fieldKey"><?php echo ENTRY_USER_NAME; ?></td>
        <td class="fieldValue"><?php echo tep_draw_input_field('user_name','','autocomplete="off"'); ?></td>
      </tr>
      <tr>
