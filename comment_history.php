...
if($_REQUEST['act']=='save') {
    ...
    //echo "<script>alert('my input is '. ".$SESSION['Sean'].")</script>";
    $new_comment = $_REQUEST['comment'];


    $price_query = tep_db_query("insert into rfq_order_comment_history(order_id,customer_id,comments) values (" . $_REQUEST['cID'] . "," . $_SESSION['customer_id'] . ",'" . $new_comment . "'" . ')');
...
?>
        ...
        <tbody>
            <?php
            $comment_his_sql = tep_db_query("select * from rfq_order_comment_history where order_id=" . $_REQUEST['cID'] . "  order by date_added asc");
